...
export class GenerationEventPort {
    #channel;
    #hostId;
    #session;
    #revision = 0n;
    #empty;
    constructor(channel, trustedHostId) {
        need(typeof channel?.request === 'function' && typeof channel?.close === 'function', 'INVALID_PORT');
        this.#channel = channel;
        this.#hostId = hex(trustedHostId, 16);
    }
    async #call(op, args, signal) {
        if (signal.aborted)
            throw new EventBindingError('CANCELLED');
        const raw = await this.#channel.request(op, copyData(args, 65536), signal);
        if (signal.aborted)
            throw new EventBindingError('CANCELLED');
        const r = obj(copyData(raw), ['protocol', 'ticket', 'result']);
        need(r.protocol === 'par-owner-event-host-0039');
        const t = obj(r.ticket, ['hostId', 'revision']);
        need(hex(t.hostId, 16) === this.#hostId);
        const rev = u64(t.revision);
        need(rev >= this.#revision);
        this.#revision = rev;
        return { result: r.result, ticket: Object.freeze({ hostId: this.#hostId, revision: t.revision }) };
    }
    async open(r, s) {
        const out = await this.#call('open', r, s);
        const value = obj(out.result, ['kind', 'context', 'sessionId', 'cursor']);
        need(value.kind === 'opened');
        this.#session = hex(value.sessionId, 16);
        return out.result;
    }
    async poll(r, s) {

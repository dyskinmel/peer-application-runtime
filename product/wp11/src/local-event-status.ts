...
export interface LocalEventStatusPanel {
 ...
 readonly scope:Readonly<{appId:string;spaceId:string;streamId:string}>;
 readonly revision:string;readonly bindingGeneration:string;readonly operationId:string|null;
 readonly connection:ClientConnection;readonly outcome:EventClientOperation['kind'];
 readonly message:string;readonly problemCode:string|null;
 readonly eventSequence:string|null;readonly localReceiptPreviouslyObserved:boolean;
 readonly controls:Readonly<{publish:boolean;inquire:boolean;rebind:boolean}>;
 readonly sharedCommit:false;readonly remoteProtection:false;readonly automaticRetry:false;
 readonly productQualified:false;
}
const messages:Readonly<Record<EventClientOperation['kind'],readonly [string,string]>>={
...
export function presentLocalEventClient(client:EventCommandClient,expectedContext:CommandContext,locale:'ja'|'en'):LocalEventStatusPanel {
 ...
 if(state.cleanup.failed)message=['以前の接続の解放を確認できません。接続の所有者を確認してください。','Previous connection cleanup is unconfirmed. Inspect the connection owner.'][index]!;
 else if(state.connection==='closed')message=['イベント接続は閉じています。過去の保存結果を現在の接続確認とは扱いません。','The event client is closed. A historical receipt is not evidence of a current connection.'][index]!;

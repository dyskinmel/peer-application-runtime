...
export interface ApplicationPin {
 readonly scope: Scope & {readonly documentId:string;readonly epoch:string;readonly schemaId:string};
 readonly storeGeneration:string;readonly certificateDigest:string;readonly streamId:string;
 readonly engineDigest:string|null;
}
export interface ApplicationObservation extends ApplicationPin {
 readonly profile:'par-application-observation-local-0036';readonly sequence:string;
 readonly revision:string;readonly observedAt:string;
 readonly authority:{readonly state:'ACTIVE';readonly controlHead:string;readonly epoch:string;readonly readerAuthorized:true};
 readonly application:{
  readonly state:'EMPTY'|'CANDIDATE_ONLY'|'CORE_RECHECK_REQUIRED'|'VALIDATED_LOCAL_VIEW';
  readonly revision:number;readonly eventDigest:string|null;readonly heads:readonly string[];
  readonly inputCount:number;readonly evidenceClass:'none'|'candidate'|'core-validated';
  readonly note:{readonly title:string;readonly body:string;readonly titleConflicts:readonly string[]}|null;
  readonly innerValidated:boolean;readonly applied:boolean;readonly recheckedCoreDigest:string|null;
 };
 readonly operation:{readonly id:string|null;readonly state:'NOT_QUERIED'|'NOT_OBSERVED'|'OBSERVED_CANDIDATE'|'OBSERVED_APPLICATION_RECORD';readonly revision:number|null;readonly eventDigest:string|null};
 readonly capabilities:{readonly observe:true;readonly inspectApplication:true;readonly sharedCommit:false;readonly sync:false;readonly apply:false};
 readonly restoreReadOnly:boolean;readonly catalogComplete:false;readonly replicationObserved:false;
 readonly globalLatestProven:false;readonly productQualified:false;readonly evidenceKind:'trusted-owner-application-observation';
}
function hex(v:unknown,n=64):asserts v is string {string(v,n);check(v.length===n&&/^[0-9a-f]+$/.test(v),'hex');}
function scopeFields(v:unknown):void {
 const s=record(v,['appId','spaceId','documentId','epoch','schemaId']);string(s.appId,128);hex(s.spaceId);hex(s.documentId);hex(s.schemaId);u64(s.epoch);check(s.epoch!=='0','epoch');
}
function pinFields(v:Record<string,unknown>):void {
 scopeFields(v.scope);hex(v.storeGeneration,32);hex(v.certificateDigest);hex(v.streamId,32);if(v.engineDigest!==null)hex(v.engineDigest);
}
export function validateApplicationPin(input:unknown):ApplicationPin {
 const p=record(input,['scope','storeGeneration','certificateDigest','streamId','engineDigest']);pinFields(p);return freeze(clone(p as unknown as ApplicationPin));
}
export function validateApplicationObservation(input:unknown):ApplicationObservation {
 ...
 pinFields(r);check(r.profile==='par-application-observation-local-0036','profile');u64(r.sequence);hex(r.revision);string(r.observedAt,24);
 const date=new Date(r.observedAt);check(Number.isFinite(date.getTime())&&date.toISOString()===r.observedAt,'timestamp');bool(r.restoreReadOnly);
 ...
 const s=r.scope as ApplicationPin['scope'];
 const a=record(r.authority,['state','controlHead','epoch','readerAuthorized']);hex(a.controlHead);u64(a.epoch);check(a.state==='ACTIVE'&&a.readerAuthorized===true&&a.epoch===s.epoch,'read authority');
 ...
 enumValue(d.state,['EMPTY','CANDIDATE_ONLY','CORE_RECHECK_REQUIRED','VALIDATED_LOCAL_VIEW']);count(d.revision,64);count(d.inputCount,128);
 list(d.heads,128,x=>hex(x));check(d.heads.length===new Set(d.heads).size&&canonical(d.heads)===canonical([...d.heads].sort()),'heads');
 enumValue(d.evidenceClass,['none','candidate','core-validated']);bool(d.innerValidated);bool(d.applied);
 if(d.state==='EMPTY') {
  check(d.revision===0&&d.eventDigest===null&&d.inputCount===0&&d.heads.length===0&&d.evidenceClass==='none','empty');
 } else {
  hex(d.eventDigest);check(d.revision>0&&d.inputCount>0&&d.heads.length>0&&d.heads.length<=d.inputCount,'frontier');
 ...
 if(d.state==='VALIDATED_LOCAL_VIEW') {
  ...
  const n=record(d.note,['title','body','titleConflicts']);string(n.title,1024,0);string(n.body,262144,0);
 ...
 const o=record(r.operation,['id','state','revision','eventDigest']);if(o.id!==null)hex(o.id,32);
 ...
 if(o.state==='NOT_QUERIED'||o.state==='NOT_OBSERVED') {
  ...
  hex(o.id,32);hex(o.eventDigest);count(o.revision,64);check(o.revision>0&&o.revision<=d.revision,'operation revision');
  check((o.state==='OBSERVED_CANDIDATE')===(d.evidenceClass==='candidate'),'operation evidence');
  if(o.revision===d.revision)check(o.eventDigest===d.eventDigest,'operation digest');
 }
 ...
 return freeze(clone(r as unknown as ApplicationObservation));
}
function bound(input:unknown,pin:ApplicationPin):ApplicationObservation {
 ...
 check(canonical(actual)===canonical(pin),'fixed scope/stream/core','APPLICATION_BINDING_MISMATCH');return r;
}
export function applicationNotice(input:unknown):Message {
 const r=validateApplicationObservation(input);
...
export function projectApplicationObservation(input:unknown,expected:unknown):NoteState {
...
export interface ApplicationObservationPort {observe(operationId:string|null):Promise<unknown>;}
...
export class ApplicationReadBinding {
 private readonly pin:ApplicationPin;
 private last:ApplicationObservation;
 private view:NoteState;
 private busy=false;
 private closed=false;
 private phase:ApplicationBindingStatus='CURRENT';
 private readonly knownOperations=new Map<string,string>();
 constructor(expected:unknown,initial:unknown,private readonly port:ApplicationObservationPort) {
  this.pin=validateApplicationPin(expected);this.last=bound(initial,this.pin);this.view=projectApplicationObservation(this.last,this.pin);this.rememberOperation(this.last);
 }
 get current():NoteState{return this.view;}
 get status():ApplicationBindingStatus{return this.phase;}
 get observation():ApplicationObservation|null{return this.phase==='CURRENT'?this.last:null;}
 private rememberOperation(r:ApplicationObservation):void {
  const o=r.operation;if(o.id===null)return;
  const known=this.knownOperations.get(o.id);
  if(known!==undefined)check(known===canonical(o),'recorded outcome changed','APPLICATION_RESULT_CHANGED');
 ...
 private mask(status:ApplicationBindingStatus):void {
  this.phase=status;
 ...
 close():void {this.closed=true;this.mask('CLOSED');}
 async refresh(operationId:string|null=null):Promise<NoteState> {
  check(!this.closed,'closed','APPLICATION_BINDING_CLOSED');check(!this.busy,'busy','APPLICATION_BINDING_BUSY');if(operationId!==null)hex(operationId,32);
  this.busy=true;this.mask('CHECKING');
  try {
   const raw=await this.port.observe(operationId);
   check(!this.closed,'closed','APPLICATION_BINDING_CLOSED');const next=bound(raw,this.pin);
   ...
   if(next.sequence===this.last.sequence||next.revision===this.last.revision)check(canonical(next)===canonical(this.last),'token reuse','REVISION_REUSED');
   ...
   if(d.revision===prev.revision) {
    ...
    if(d.note!==null&&prev.note!==null)check(canonical(d.note)===canonical(prev.note),'same materialization','APPLICATION_FRONTIER_CHANGED');
   }
   this.rememberOperation(next);
   this.last=next;this.view=projectApplicationObservation(next,this.pin);this.phase='CURRENT';return this.view;
  } catch(error) {
   if(!this.closed)this.mask('UNAVAILABLE');
   throw error;

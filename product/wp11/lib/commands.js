...
export function prepareCommand(current, input) {
    ...
    const a = present(s).actions.find(a => a.kind === c.kind);
    check(a, 'action not offered', 'ACTION_NOT_OFFERED');
    check(a.enabled, a.disabledReason ?? 'disabled', 'ACTION_DISABLED');
    if (a.confirmation !== 'none') {
        check(c.confirmation !== null, 'preview required', 'CONFIRMATION_REQUIRED');
        check(canonical(c.confirmation) === canonical(matchingPreview(s, c.kind)), 'preview changed', 'STALE_CONFIRMATION');
    }
    else
        check(c.confirmation === null, 'unexpected confirmation');
    if (c.kind === 'inspect-operation') {
        const expected = s.surface === 'rpc' ? s.rpc.operationId : s.local.operationId;
        check(c.operationId === expected, 'original operation required', 'OPERATION_MISMATCH');
    }
...
export function advanceSnapshot(previous, next) {
    ...
    const equal = canonical(a) === canonical(b);
    const x = BigInt(a.sequence), y = BigInt(b.sequence);
    check(y >= x, 'older snapshot', 'STALE_VIEW');
    if (y === x || a.revision === b.revision)
        check(equal, 'revision reused for changed observation', 'REVISION_REUSED');
    return b;
}

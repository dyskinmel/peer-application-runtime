...
TASKS=['H0-SELFTEST','G0-WIRE-LOCAL','G0-STORE-LOCAL','G0-CRYPTO-LOCAL','SPACE-AUTH-LOCAL','AUTH-STORE-LOCAL','BLOB-STORE-LOCAL','BLOB-MANIFEST-LOCAL','RECOVERY-CLOSURE-LOCAL','KEEPER-RETENTION-LOCAL','KEEPER-GC-LOCAL','KEEPER-REPAIR-LOCAL','KEEPER-SERVICE-LOCAL','KEEPER-SERVICE-UPLOAD-LOCAL','KEEPER-UPLOAD-RETIRE-LOCAL','UPLOAD-REPLAY-WINDOW-LOCAL']
INDEX=ROOT/'.harness/upload-window-workflow-index.json'
def step(tid):
    ...
    cp=checkpoint(ROOT,s['id'],'Read plan/NEXT_UPLOAD_WINDOW_HOST.ja.md. Signed local generation closure and bounded terminal record compaction are implemented; compatible IPC host, public/native transport, CRDT and writable recovery remain unverified.')

#!/usr/bin/env python3
"""Synchronous fresh checks and checkpoint/resume; not a background agent service."""
...
def main():
    ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--include-wire',action='store_true');ap.add_argument('--include-store',action='store_true');args=ap.parse_args()
    tasks=['H0-SELFTEST']+(['G0-WIRE-LOCAL'] if args.include_wire else [])+(['G0-STORE-LOCAL'] if args.include_store else [])+['G0-CRYPTO-LOCAL','SPACE-AUTH-LOCAL']
    steps=[]
    for tid in tasks:
        ...
        c=checkpoint(ROOT,s['id'],'Read plan/NEXT_AUTH_STORE.ja.md. Pure authority state is not atomic Store authorization. Keep product gates open; verify pins on each host.')
        re=resume(ROOT,c['id'])

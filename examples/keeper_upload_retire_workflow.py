...
TASKS=['H0-SELFTEST','G0-WIRE-LOCAL','G0-STORE-LOCAL','G0-CRYPTO-LOCAL','SPACE-AUTH-LOCAL','AUTH-STORE-LOCAL','BLOB-STORE-LOCAL','BLOB-MANIFEST-LOCAL','RECOVERY-CLOSURE-LOCAL','KEEPER-RETENTION-LOCAL','KEEPER-GC-LOCAL','KEEPER-REPAIR-LOCAL','KEEPER-SERVICE-LOCAL','KEEPER-SERVICE-UPLOAD-LOCAL','KEEPER-UPLOAD-RETIRE-LOCAL']
INDEX=ROOT/'.harness/keeper-upload-retire-workflow-index.json'
def step(tid):
    ...
    cp=checkpoint(ROOT,s['id'],'Read plan/NEXT_UPLOAD_REPLAY_WINDOW.ja.md. Explicit local staging retirement is implemented; record compaction, compatible host upgrade, public/native transport, CRDT and writable recovery remain unverified.')

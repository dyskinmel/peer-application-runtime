...
with KeeperRepair(q['keeper'],p,bytes.fromhex(q['seed']),authority_from(decode(bytes.fromhex(q['authority']))),quota_bytes=q['quota'],allow_unpatched_sqlite=True) as k:
    with ReplaySpool(k,q['spool'],bytes.fromhex(q['store'])) as g:
        with ControlledHost(k,g,q['read'],q['upload'],q['control'],q['jobs'],q['journal'],bytes.fromhex(q['operator']),q['revision']) as host:
            stage=RetiringSubmissions(host.controller,q['stage'],observer=hit,migrate_legacy=True)
            try:
                raw=bytes.fromhex(q['request']);op=q['operation']
                if op=='retire':stage.retire(raw)
                elif op=='rebind':stage.rebind(raw)
                elif op=='reconcile':stage.reconcile_registration(raw)
                elif op!='migration':raise RuntimeError('unknown synthetic operation')
            finally:stage.close()
raise RuntimeError('selected boundary not reached')

"""Separate test-only donor. Job contains public synthetic fixture seeds only."""
import sys,json
...
from par_window_host.client import Client
...
def main():
    ...
    c=Client(Path(j['socket']),p,b('keeper'),b('seed'),b('cap'),b('store'),b('window'))
    lid,receipt=c.upload_bundle(b('index'),pin_from(decode(b('pin'))),{bytes.fromhex(k):bytes.fromhex(v) for k,v in j['objects'].items()},30,hashed('test/window-upload',[b('index')]))

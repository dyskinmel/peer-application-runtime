#!/usr/bin/env python3
"""Local opaque Keeper retention checks, never a product qualification runner."""
from __future__ import annotations
import argparse,os,sys,unittest
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];sys.dont_write_bytecode=True
sys.path.insert(0,str(ROOT))
from tools.check_recovery import GROUPS as RECOVERY_GROUPS
for path in ('experiments/keeper-retention','tests/product/keeper-retention'):
    sys.path.insert(0,str(ROOT/path))
from harness.common import atomic_json
from tools.test_runner import RecordedResult
GROUPS={'contract':'test_lease_contract','store':'test_keeper_store','lifecycle':'test_keeper_lifecycle','faults':'test_keeper_faults'}
def suite(group='all'):

#!/usr/bin/env python3
"""Private Linux data + owner-control host. Only pre-registered jobs; no shell/API injection."""
import sys
...
for n in ('host-management-jobs','host-job-scheduler','host-job-control'):sys.path.insert(0,str(ROOT/'experiments'/n))
def main():
    ...
    from par_job_control import protocol as c
    ...
    for name in ('socket','upload-socket','control-socket','jobs','control-state'):ap.add_argument('--'+name,type=Path,required=True)
    ...
    try:
        ...
        jp,cp=pin(a.jobs_pin_file),pin(a.control_pin_file)
        ...
        with owned_verified(a.root,a.spool,cfg,p,bytes(secret),mode=a.verification_mode,allow_unpatched_sqlite=a.allow_unpatched_sqlite) as (k,g,cache,startup):
            ...
            with ControlledHost(k,g,a.socket,a.upload_socket,a.control_socket,a.jobs,a.control_state,public,a.control_revision,expected_jobs_pin=jp,expected_control_pin=cp,

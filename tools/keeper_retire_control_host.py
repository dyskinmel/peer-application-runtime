#!/usr/bin/env python3
"""Private Linux registration host accepting retirement schema2. Explicit domain-separated retirement control on the same endpoint."""
import sys
...
for n in ('host-management-jobs','host-job-scheduler','host-job-control','host-job-submit','job-submission-retire','job-submission-retire-control'):sys.path.insert(0,str(ROOT/'experiments'/n))
def main():
    ...
    try:
        ...
        with owned_verified(a.root,a.spool,cfg,p,bytes(secret),mode=a.verification_mode,allow_unpatched_sqlite=a.allow_unpatched_sqlite) as (k,g,cache,startup):
            ...
            with RetirementControlHost(k,g,a.socket,a.upload_socket,a.control_socket,a.submit_socket,a.jobs,a.control_state,a.submissions,public,a.control_revision,expected_jobs_pin=jp,expected_control_pin=cp,migrate_submissions=a.migrate_submissions,expected_submission_pin=sp,

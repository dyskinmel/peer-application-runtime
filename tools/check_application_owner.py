#!/usr/bin/env python3
"""Exact local experimental anchored owner checks; no product qualification.
...
GROUPS={'owner':'test_application_owner','process':'test_application_process'}
ALL=(*GROUPS,'node')
TYPE_ID='application.owner.types.contracts'
...
def node_ids():
    cp=subprocess.run([shutil.which('node')or'node',str(ROOT/'tests/product/application-owner/node_contracts.mjs'),'--list'],capture_output=True,text=True,timeout=15,check=True,env=clean_env())
    return sorted(json.loads(cp.stdout)+[TYPE_ID])

def node_run():
    with tempfile.TemporaryDirectory(prefix='par-application-owner-types-')as tmp:
        ...
        cp=subprocess.run([compiler['node'],str(ROOT/'tests/product/application-owner/node_contracts.mjs')],capture_output=True,text=True,timeout=45,env=env)
        ...
        cp=subprocess.run([compiler['tsc'],'--noEmit','--target','ES2022','--module','NodeNext','--moduleResolution','NodeNext','--strict','--exactOptionalPropertyTypes','--noUncheckedIndexedAccess',str(ROOT/'tests/product/application-owner/type_contract.mts')],capture_output=True,text=True,timeout=45,env=clean_env())
...
def main():
    ...
    try:
        ...
        print(json.dumps({'result':'PASS'if good else'FAIL','registered_cases':len(ids),'executed_cases':len(actual),'real_core_executed':False,'browser_executed':False,'product_qualified':False,'local_experimental_apply_exposed':True,'production_apply_exposed':False}));return 0 if good else 1

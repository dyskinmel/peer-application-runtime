#!/usr/bin/env python3
"""Exact-ID tests; real local TLS, not public transport/product qualification."""
from __future__ import annotations
import argparse,json,os,sys,unittest,shutil,ssl
...
GROUPS={'tls':'test_tls','frames':'test_frames','read':'test_read_session','process':'test_process_tls','adapters':'test_tls_adapters','hardening':'test_tls_hardening'}
def suite(group='all'):
...
def main():
    a=argparse.ArgumentParser(description=__doc__);a.add_argument('--list',action='store_true');a.add_argument('--suite',choices=['all',*GROUPS],default='all');ns=a.parse_args()
    tests=suite(ns.suite);expected=sorted(t.id() for t in flat(tests))
    ...
    actual=[c['id'] for c in r.cases]
    good=bool(actual) and len(set(actual))==len(actual) and sorted(actual)==expected and all(c['status']=='PASS' for c in r.cases) and r.wasSuccessful()
    print(json.dumps({'result':'PASS' if good else 'FAIL','registered_cases':len(expected),'executed_cases':len(actual),'public_network_executed':False,'independent_review':'NOT_RUN'}))

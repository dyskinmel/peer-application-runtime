#!/usr/bin/env python3
"""Exact private owner, three-process and typed controls checks; no apply exposed.
...
for p in [ROOT,ROOT/'tests/product/fetch-owner',ROOT/'tests/product/fetch-bridge',ROOT/'tests/product/document-apply',ROOT/'tests/product/secure-fetch',ROOT/'tests/product/secure-transport',ROOT/'tests/product/causal-exchange',ROOT/'tests/product/auth-store',ROOT/'tests/product/space-auth']+[p for p in(ROOT/'experiments').iterdir()if p.is_dir()]:sys.path.insert(0,str(p))
...
GROUPS={'owner':'test_owner','hardening':'test_owner_hardening','process':'test_owner_process'}
ALL=(*GROUPS,'node')
TYPE_ID='fetch.owner.types.contracts'
...
def node_ids():
    cp=subprocess.run([shutil.which('node')or'node',str(ROOT/'tests/product/fetch-owner/node_contracts.mjs'),'--list'],capture_output=True,text=True,timeout=15,check=True,env=clean_env())
    return sorted(json.loads(cp.stdout)+[TYPE_ID])

def node_run():
    with tempfile.TemporaryDirectory(prefix='par-fetch-owner-types-')as tmp:
        dest=Path(tmp);compiler=build(dest)
        fixture=dest/'fixture.json';result=dest/'node-result.json'
        cp=subprocess.run([sys.executable,'-I','-S','-B',str(ROOT/'tests/product/fetch-owner/export_fixture.py')],capture_output=True,text=True,timeout=20,env=clean_env())
        if cp.returncode:raise RuntimeError('PYTHON_FIXTURE_FAILED\n'+cp.stderr)
        json.loads(cp.stdout);fixture.write_text(cp.stdout)
        env=clean_env();env.update(PAR_ROOT=str(ROOT),PAR_PRESENTER_BUILD=str(dest),PAR_OWNER_FIXTURE=str(fixture),HARNESS_RESULT_PATH=str(result),HARNESS_NONCE=os.environ.get('HARNESS_NONCE','standalone'))
        cp=subprocess.run([compiler['node'],str(ROOT/'tests/product/fetch-owner/node_contracts.mjs')],capture_output=True,text=True,timeout=45,env=env)
        ...
        cp=subprocess.run([compiler['tsc'],'--noEmit','--target','ES2022','--module','NodeNext','--moduleResolution','NodeNext','--strict','--exactOptionalPropertyTypes','--noUncheckedIndexedAccess',str(ROOT/'tests/product/fetch-owner/type_contract.mts')],capture_output=True,text=True,timeout=45,env=clean_env())
...
def main():
    ...
    try:
        ...
        print(json.dumps({'result':'PASS'if good else'FAIL','registered_cases':len(ids),'executed_cases':len(actual),'real_core_executed':False,'browser_executed':False,'product_qualified':False,'apply_operation_exposed':False}));return 0 if good else 1

#!/usr/bin/env python3
"""Optional bounded positive signature reuse on the existing private Linux host."""
import sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];sys.dont_write_bytecode=True;sys.path.insert(0,str(ROOT))
for name in ('g0-wire','g0-store','g0-crypto','space-auth','auth-store','blob-store','blob-manifest','recovery-closure','keeper-retention','keeper-gc','keeper-repair','keeper-service','keeper-upload','keeper-upload-retire','upload-replay-window','upload-window-host','host-verified-snapshot'):
...
def main():
    ...
    from par_verified_host.host import owned_verified,observe_host
    ...
    ap.add_argument('--cache-max-entries',type=int,default=512)
    ap.add_argument('--cache-max-key-bytes',type=int,default=4*1024*1024)
    ap.add_argument('--cache-max-message-bytes',type=int,default=65536)
    ...
    try:
        ...
        with owned_verified(a.root,a.spool,cfg,provider,bytes(secret),mode=a.verification_mode,max_entries=a.cache_max_entries,max_key_bytes=a.cache_max_key_bytes,max_message_bytes=a.cache_max_message_bytes,allow_unpatched_sqlite=a.allow_unpatched_sqlite) as (keeper,gateway,cached,startup):
            secret[:]=bytes(32)
            with ReadServer(keeper,a.socket,max_connections=a.max_connections,deadline_ms=a.deadline_ms) as read,Server(keeper,gateway,a.upload_socket,max_connections=a.max_connections,deadline_ms=a.deadline_ms) as upload:
                print(json.dumps({'state':'READY','profile':PROFILE,'keeper':keeper.public.hex(),'store':gateway.store_id.hex(),'sequence':gateway.pin()[1],'phase':gateway.phase,'product_qualified':False,'verification_mode':a.verification_mode,'startup_verification':startup}),flush=True)
                while running:
                    observe_host(cached,keeper,gateway)
                    read.poll(.005)
                    observe_host(cached,keeper,gateway)
                    upload.poll(.005)
                    observe_host(cached,keeper,gateway)
                print(json.dumps({'state':'STOPPING','read':read.diagnostics(),'upload':upload.diagnostics(),'startup_verification':startup,'verification':cached.statistics()}),flush=True)
        return 0

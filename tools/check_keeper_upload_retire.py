#!/usr/bin/env python3
"""Run explicit local staging retirement tests, not product qualification."""
...
from tools.check_keeper_upload import GROUPS as UPLOAD_GROUPS
for p in ('experiments/keeper-upload-retire','tests/product/keeper-upload-retire'):sys.path.insert(0,str(ROOT/p))
from harness.common import atomic_json
from tools.test_runner import RecordedResult
GROUPS={'contract':'test_retire_contract','lifecycle':'test_retire_lifecycle','audit':'test_retire_audit','faults':'test_retire_faults'}
def suite(group='all'):

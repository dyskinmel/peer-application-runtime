#!/usr/bin/env python3
"""Same-endpoint retirement control; synthetic local contract and process tests."""
import argparse, os, sys, unittest
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
sys.dont_write_bytecode=True; sys.path.insert(0,str(ROOT))
from tools.check_submit_retire import GROUPS as PREVIOUS_GROUPS
for n in ('experiments/job-submission-retire-control','tests/product/job-submission-retire-control'):
    sys.path.insert(0,str(ROOT/n))
from harness.common import atomic_json
from tools.test_runner import RecordedResult
GROUPS={'contract':'test_retire_control_contract','socket':'test_retire_control_socket',
        'restart':'test_retire_control_restart','process':'test_retire_control_process'}
def suite(group='all'):

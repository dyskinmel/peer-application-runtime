#!/usr/bin/env python3
"""Exact-ID admission checks, NOT a passing real Autome rge/merge qualification."""
...
for p in [ROOT,ROOT/'tests/product/shared-document',ROOT/'tests/product/auth-store',ROOT/'tests/product/space-auth']+[p for p in (ROOT/'experiments').iterdir() if p.is_dir()]:sys.path.insert(0,str(p))
from harness.common import atomic_json,clean_env
from tools.test_runner import RecordedResult
GROUPS={'contracts':'test_contracts','store':'test_writer','loader':'test_core_port','node':None}
def flatten(s):
 for x in s:
...
def inventory(group):
 ...
 return sorted(json.loads(subprocess.check_output(['node',str(ROOT/'tests/product/shared-document/core_contracts.mjs'),'--list'],env=clean_env(),cwd=ROOT,text=True,timeout=15)))
def main():
 ...
 for g in groups:
  if g!='node':
   ...
   with tempfile.TemporaryDirectory(prefix='par-shared-check-') as p:
    ...
    cp=subprocess.run(['node',str(ROOT/'tests/product/shared-document/core_contracts.mjs')],capture_output=True,text=True,cwd=ROOT,env=env,timeout=30)
 ...
 print(json.dumps({'result':'PASS' if good else 'FAIL','cases':len(results),'scope':'CONTRACTS_AND_REAL_STORE_WITH_EXPLICIT_SYNTHETIC_SEMANTIC_PORT','real_core_executed':False,'product_qualified':False}));return 0 if good else 1

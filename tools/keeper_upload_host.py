#!/usr/bin/env python3
"""Run an existing schema3 Keeper on a private Linux socket; private upload and separate read sockets; no public networking."""
import sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];sys.dont_write_bytecode=True;sys.path.insert(0,str(ROOT))
for name in ('g0-wire','g0-store','g0-crypto','space-auth','auth-store','blob-store','blob-manifest','recovery-closure','keeper-retention','keeper-gc','keeper-repair','keeper-service','keeper-upload'):
...
def main():
    ...
    from par_keeper_upload.server import Server as UploadServer
    ...
    p.add_argument('--upload-socket',type=Path,required=True)
    ...
    try:
        ...
        with KeeperRepair(a.root,provider,bytes(secret),auth,quota_bytes=cfg[2],max_leases=cfg[3],max_operations=cfg[4],allow_unpatched_sqlite=a.allow_unpatched_sqlite) as keeper:
            ...
            with Spool(keeper,keeper.root/'incoming-upload') as spool, Server(keeper,a.socket,max_connections=a.max_connections,deadline_ms=a.deadline_ms) as server, UploadServer(keeper,spool,a.upload_socket,max_connections=a.max_connections,deadline_ms=a.deadline_ms) as upload:
                print(json.dumps({'state':'READY','profile':'keeper-upload-local-v1','keeper':keeper.public.hex(),'product_qualified':False}),flush=True)
                while running:
                    server.poll(.005);upload.poll(.005)

#!/usr/bin/env python3
"""Run bounded private job-control checks; no production qualification."""
...
from tools.check_job_scheduler import GROUPS as PREVIOUS_GROUPS
for name in ('experiments/host-job-control','tests/product/host-job-control'):
    sys.path.insert(0,str(ROOT/name))
from harness.common import atomic_json
from tools.test_runner import RecordedResult
GROUPS={'contract':'test_control_contract','lifecycle':'test_control_lifecycle','socket':'test_control_socket','restart':'test_control_restart','hardening':'test_control_hardening'}
def suite(group='all'):

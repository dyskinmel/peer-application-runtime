#!/usr/bin/env python3
"""Closed upload-window candidate tests; never product qualification."""
...
from tools.check_keeper_upload_retire import GROUPS as RETIRE_GROUPS
for p in ('experiments/upload-replay-window','tests/product/upload-replay-window'):sys.path.insert(0,str(ROOT/p))
from harness.common import atomic_json
from tools.test_runner import RecordedResult
GROUPS={'contract':'test_window_contract','lifecycle':'test_window_lifecycle','audit':'test_window_audit','faults':'test_window_faults','hardening':'test_window_hardening'}
def suite(group='all'):

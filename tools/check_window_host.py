#!/usr/bin/env python3
"""Private replay-window host checks; not native or product qualification."""
...
from tools.check_upload_window import GROUPS as WINDOW_GROUPS
for p in ('experiments/upload-window-host','tests/product/upload-window-host'):sys.path.insert(0,str(ROOT/p))
from harness.common import atomic_json
from tools.test_runner import RecordedResult
GROUPS={'contract':'test_host_contract','config':'test_host_config','service':'test_host_service','process':'test_host_process','roundtrip':'test_host_roundtrip','faults':'test_host_faults','hardening':'test_host_hardening'}
def suite(group='all'):

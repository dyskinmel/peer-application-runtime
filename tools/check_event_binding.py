#!/usr/bin/env python3
"""Exact-ID Python owner / TypeScript subscription / actual process integration tests."""
...
for p in [ROOT,ROOT/'tests/product/event-binding',ROOT/'tests/product/wp10',ROOT/'tests/product/auth-store',ROOT/'tests/product/space-auth']+list((ROOT/'experiments').iterdir()):sys.path.insert(0,str(p))
from harness.common import atomic_json,clean_env
from tools.test_runner import RecordedResult
GROUPS=('owner','node','integration','types')
def flatten(s):
 for x in s:
  if isinstance(x,unittest.TestSuite):yield from flatten(x)
  else:yield x
def inventory(group):
 if group=='owner':return sorted(x.id() for x in flatten(unittest.defaultTestLoader.loadTestsFromName('test_owner_port.OwnerPortTests')))
 if group=='types':return ['event-binding.types.contract']
 script='run.mjs' if group=='node' else 'integration.mjs'
 return sorted(json.loads(subprocess.check_output(['node',str(ROOT/'tests/product/event-binding'/script),'--list'],cwd=ROOT,env=clean_env(),text=True,timeout=15)))
def build(dest):
...
def main():
 ...
 cases=[]
 if 'owner'in groups:
  r=unittest.TextTestRunner(verbosity=2,resultclass=RecordedResult).run(unittest.defaultTestLoader.loadTestsFromName('test_owner_port.OwnerPortTests'));cases+=r.cases
 if set(groups)-{'owner'}:
  with tempfile.TemporaryDirectory(prefix='par-sdk-test-') as tmp:
   ...
   for group in groups:
    ...
    if group=='types':
     cp=subprocess.run([compiler['tsc'],'--noEmit','--target','ES2022','--module','NodeNext','--moduleResolution','NodeNext','--strict','--exactOptionalPropertyTypes','--noUncheckedIndexedAccess',str(ROOT/'tests/product/event-binding/type_contract.mts')],cwd=ROOT,env=clean_env(),capture_output=True,text=True,timeout=45)
     print(cp.stdout+cp.stderr,end='');cases.append({'id':'event-binding.types.contract','status':'PASS' if cp.returncode==0 else 'FAIL'});continue
    ...
    script='run.mjs' if group=='node' else 'integration.mjs'
    cp=subprocess.run([compiler['node'],str(ROOT/'tests/product/event-binding'/script)],cwd=ROOT,env=env,capture_output=True,text=True,timeout=90)
 ...
 ok=bool(cases)and all(x['status']=='PASS'for x in cases);print(json.dumps({'result':'PASS'if ok else 'FAIL','scope':'LOCAL_TYPED_EVENT_PORT_REAL_SQLITE_TEST_BRIDGE_NO_PRODUCTION_TRANSPORT','cases':len(cases)}));return 0 if ok else 1

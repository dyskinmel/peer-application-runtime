"""Fourth bounded private endpoint, composing the existing three-endpoint host."""
...
class SubmissionHost:
    def __init__(self,keeper,gateway,read_path,upload_path,control_path,submit_path,jobs_root,control_root,submission_root,controller_public,controller_revision,*,submit_deadline_ms=5000,submit_max_connections=4,submission_observer=None,**kwargs):
        ...
        try:
            ...
            self.submissions=Submissions(self.controller,submission_root,observer=submission_observer);self.stack.callback(self.submissions.close)
    ...
    def diagnostics(self):
        d=self.controlled.diagnostics();d['submission']=self.submit_server.diagnostics();d['submission_scope']='REGISTRATION_ONLY';return d
    def close(self):

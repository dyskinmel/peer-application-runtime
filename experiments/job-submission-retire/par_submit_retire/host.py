"""Compatible registration host for schema2. Retirement remains owner-API only."""
...
from .store import RetiringSubmissions
from par_job_submit.server import Server
from par_job_submit.protocol import E
class RetiringSubmissionHost:
    def __init__(self,keeper,gateway,read_path,upload_path,control_path,submit_path,jobs_root,control_root,submission_root,controller_public,controller_revision,*,submit_deadline_ms=5000,submit_max_connections=4,submission_observer=None,migrate_submissions=False,expected_submission_pin=None,**kwargs):
        ...
        try:
            ...
            self.submissions=RetiringSubmissions(self.controller,submission_root,observer=submission_observer,migrate_legacy=migrate_submissions,expected_pin=expected_submission_pin);self.stack.callback(self.submissions.close)
